                                        ...
                                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                            ...
                                            <div className="flex-1 space-y-1">
                                                <div className="flex items-center gap-2 text-sm font-medium">
                                                    ...
                                                    <span className={`text-muted-foreground ${item.paid ? 'dark:text-green-300' : ''}`}>• Thn {item.year}</span>
                                                </div>

                                                {/* Extra Details: Original Name, Persil, Blok */}
                                                <div className={`flex flex-wrap gap-x-4 gap-y-1 text-xs ml-1 ${item.paid ? 'text-muted-foreground dark:text-green-300' : 'text-muted-foreground'}`}>
                                                    {item.original_name && (
                                                    ...
                                                    {item.blok && (
                                                        <span className={`px-1.5 rounded border ${item.paid ? 'bg-slate-50 border-slate-100 dark:bg-green-900/50 dark:border-green-800' : 'bg-slate-50 border-slate-100 dark:bg-slate-900 dark:border-slate-800'}`}>
                                                            Blok: {item.blok}
                                                        </span>
                                                    )}
                                                    {item.persil && (
